...
class Mobile_Detect
{
	...
	protected static $operatingSystems = array(
		...
		'Windows Phone'   => 'Windows Phone 8.1|Windows Phone 8.0|Windows Phone OS|XBLWP7|ZuneWP7|Windows NT 6.[23]; ARM;',
	 ...
	 * @return bool
	 */
	protected function isMobile($userAgent = null, $httpHeaders = null)
	{
	 ...
	 * @return bool
	 */
	protected function isTablet($userAgent = null, $httpHeaders = null)
	{
		// Check specifically for cloudfront headers if the useragent === 'Amazon CloudFront'
		if ($this->getUserAgent() === 'Amazon CloudFront') {
			$cfHeaders = $this->getCfHeaders();
	...
	/**
	 * Get the platform
	 * Return platform name (string) if any type of browser is detected.
	 *
	 * @param  string $userAgent   deprecated
	 * @param  array  $httpHeaders deprecated
	 * @return bool|string
	 */
	public function getPlatform($userAgent = null, $httpHeaders = null){
		foreach(self::$operatingSystems as $os => $_regex){
			if ($this->match($_regex, $userAgent)) {
				return $os;
			}
		}
	...
	/**
	 * Get the browser
	 * Return browser_name (string) if any type of browser is detected.
	 *
	 * @param  string $userAgent   deprecated
	 * @param  array  $httpHeaders deprecated
	 * @return bool|string
	 */
	public function getBrowser($userAgent = null, $httpHeaders = null){
		foreach(self::$browsers as $browser => $_regex){
			if ($this->match($_regex, $userAgent)) {
				return $browser;
			}
		}
	...
	{
		$match = (bool) preg_match(sprintf('#%s#is', $regex), (false === empty($userAgent) ? $userAgent : $this->userAgent), $matches, PREG_OFFSET_CAPTURE);
